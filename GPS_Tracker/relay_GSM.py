...
# SIM800L
gsm_module = UART(1, baudrate=9600, tx=Pin(8), rx=Pin(9))
...
def public_to_server():
    # Initialize SIM800L
    try:
        while True:
            if gsm_module.any():  # Check if there's data available
                gsm_module.write('AT+CID')
                incoming_data = lora_uart.read()  # Read the data
                if incoming_data:
                    print(f"Received: {incoming_data.decode('utf-8').strip()}")
                    time.sleep(0.1)
                gsm_module.write('AT+HTTPINIT')
                gsm_module.write("AT+HTTPACTION=1") # POST Method
                gsm_module.write("AT+HTTPPARA=<HTTPParamTag>,<HTTPParamValue>")
    ...
    # send HTTP 'POST' request with coord payload 
    # close
